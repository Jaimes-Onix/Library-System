      ...
      <div className="flex items-center justify-between px-10 h-full">
        ...
        <div className="flex items-center gap-4 min-w-[240px]">
          ...
          >
            ...
            <span className={`font-bold tracking-tight text-xl hidden sm:block transition-colors duration-300 ${isDark ? 'text-white group-hover:text-blue-400' : 'text-gray-900 group-hover:text-blue-600'}`}>
              Digital FlipBook
            </span>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-10">
            {navLinks.map((link) => (
              <button
                ...
                className={`text-sm font-black uppercase tracking-widest transition-colors ${view === link.view ? 'text-blue-500' : (isDark ? 'text-gray-400 hover:text-white' : 'text-gray-500 hover:text-gray-900')}`}
              >
                {link.label}
              </button>
            ))}
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-3 animate-in fade-in duration-500">
              ...
              <div 
                ...
                className={`group relative w-10 h-10 rounded-full border flex items-center justify-center text-xs font-black cursor-pointer transition-all hover:ring-4 hover:ring-blue-500/20 ${isDark ? 'bg-zinc-800 border-white/10 text-white' : 'bg-gray-100 border-gray-200 text-gray-900'}`}
              >
                ...
                <div className="absolute -bottom-1 -right-1 bg-blue-600 text-white p-1 rounded-full scale-0 group-hover:scale-100 transition-transform shadow-lg border-2 border-white dark:border-black">
            ...
            <>
              ...
              <button 
                onClick={onAuth}
                className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm transition-all hover:bg-blue-500 active:scale-95 shadow-lg shadow-blue-500/20"
              >
                Get Started
              </button>
            </>
